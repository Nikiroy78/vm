        ...
        switch (this->bytes.at(0)) {
            ...
            case byte(0x04): // Системный ввод-вывод
                switch (this->bytes.at(1)) {
                    ...
                    case byte(0x01): {  // Вывод в консоль
                        STDIN si(this->bytes, this->globalStack);
                        return si.checkCorrectSyntax();
                    }
                }
            default:
                return false;
...
void CommandExecutor::execute () {
    if (this->isCommandDefined) {
        if (this->bytes.at(0) == byte(0x00)) {  // Работа с функциями
            switch (this->bytes.at(1)) {
                case byte(0x00): { // Заголовок функции
                    VOID_HEADER vh(this->bytes, this->globalStack);
                    vh.exec();
            ...
            switch (this->bytes.at(1)) {
                ...
                case byte(0x01): {
                    STDIN si(this->bytes, this->globalStack);
                    si.exec();
                    while (si.localStack.size() != 0) {
                        vector<byte> bytes = si.localStack.get();
                        this->insiderStack.push(bytes);
                        si.localStack.rm();
                    }
                    break;
